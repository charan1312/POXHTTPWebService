...
public class FoodItemService {
    ...
    private Map<Integer,FoodItem> foodItemsList = new HashMap<Integer,FoodItem>();
        ...
        if(input.substring(1,13).compareTo("NewFoodItems") == 0) {
            ...
            try {
                ...
                for (FoodItem fi : newFoodItems.getNewFoodItems()) {
                    ...
                    String country = fi.getCountry();
                    double price = fi.getPrice();
                    String desc = fi.getDescription();
                    
                    if(name.trim().isEmpty() || name == null || category.trim().isEmpty() || category == null || country.trim().isEmpty() || country == null || price==0 || desc.trim().isEmpty() || desc==null )
    ...
    private String marshalAddedFoodToXML(FoodItemAdded foodAdded) {
        ...
        try {
            ...
            FoodItemData foodItemData1 = new FoodItemData();
            List<FoodItem> allFood = new ArrayList<FoodItem>();
            for(FoodItem f :foodItemsList.values())
                allFood.add(f);
            foodItemData1.setFoodItemsList(allFood);
            //System.out.println("ALL FOOD SIZE:"+ allFood.size());
            File file = new File(this.getClass().getClassLoader().getResource("FoodItemData.xml").getFile());
            Marshaller fileWriter = JAXBContext.newInstance(FoodItemData.class).createMarshaller();
            fileWriter.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
            fileWriter.marshal(foodItemData1,file);
    ...
    private int getFoodItemId(String name, String cate) {
        ...
        while(it.hasNext()) {
            ...
            if( fi.getName().equalsIgnoreCase(name) && fi.getCategory().equalsIgnoreCase(cate) )
                return fi.getId();
        }
        return 0;
    }
